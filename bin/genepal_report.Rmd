...
```{r busco_results, eval=(exists("busco_merged_final_df") && nrow(busco_merged_final_df) > 0), results='asis'}
cat("## BUSCO")
...
```{r orthofinder_results, eval=(exists("final_orthstats") && !is.null(final_orthstats)), results='asis'}
cat("## OrthoFinder")
...
```{r pangenome_matrix_heading, eval=(exists("n0_df") && !is.null(n0_df$heatmap)), results='asis'}
...
```{r pheatmap, eval=(exists("n0_df") && !is.null(n0_df$heatmap)), results='hide', fig.align='center', fig.cap="Heatmap showing number of proteins present in each orthocluster (clusters where all individuals have 1 copy are excluded). Columns = Orthologue cluster, Row = Individual", fig.width=7, fig.height=7, dpi=150}
...
```{r benchmarking_statistics}
benchmarking_statistics <- parse_gff_compare_folder("results/benchmark/")
```


```{r eval=(exists("benchmarking_statistics") && !is.null(benchmarking_statistics)), results='asis'}
...
```{r print_parameters, eval=(file.exists("results/pipeline_info/workflow_summary_mqc.yaml")), results='asis'}
...
```{r print_versions, eval=(file.exists("results/pipeline_info/genepal_software_mqc_versions.yml")), results='asis'}
...
```{r print_wf_description, eval=(file.exists("results/pipeline_info/methods_description_mqc.yaml")), results='asis'}
